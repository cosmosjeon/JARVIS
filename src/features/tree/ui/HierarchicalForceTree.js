...
import { generateNodeTitle } from 'features/tree/services/NodeTitleService';
import {
  isNodeTitleManual,
  markNodeTitleAuto,
  markNodeTitleManual,
} from 'features/tree/utils/nodeTitlePreferences';
...
const resizeCompactWindowForDropdown = useCallback((isOpen, options = {}) => {
  ...
  const handleAnswerComplete = useCallback(async (nodeId, payload = {}) => {
    ...
    const manualPreferenceActive = isNodeTitleManual(activeTreeId, nodeId);
    const canOverrideKeyword = !shouldPreserveKeyword
      && nodeSnapshot.titleManuallyEdited !== true
      && !manualPreferenceActive;
    const shouldAttemptAutoTitle = canOverrideKeyword
      && nodeSnapshot.autoTitleGenerated !== true
      && nodeSnapshot.autoTitleAttempted !== true;

    let resolvedKeyword = nodeSnapshot.keyword || '';
    let autoGeneratedTitle = null;
    ...
    if (shouldAttemptAutoTitle) {
      try {
        ...
        if (generation?.title) {
          autoGeneratedTitle = generation.title;
          resolvedKeyword = autoGeneratedTitle;
          markNodeTitleAuto(activeTreeId, nodeId, autoGeneratedTitle);
        }
      } catch (titleError) {
        if (process.env.NODE_ENV === 'development') {
          // eslint-disable-next-line no-console
          console.debug('[HierarchicalForceTree] node title generation fallback', titleError);
        }
      }
    }

    if (!autoGeneratedTitle && canOverrideKeyword && isPlaceholderNode && question) {
      ...
      if (shouldAttemptAutoTitle) {
        markNodeTitleAuto(activeTreeId, nodeId, resolvedKeyword);
      }
    }
    ...
    setData((prev) => {
      const nextNodes = prev.nodes.map((node) => {
        ...
        const finalKeyword = (resolvedKeyword || node.keyword || '').trim().slice(0, 48);
        const autoTitleAttempted = shouldAttemptAutoTitle || node.autoTitleAttempted === true;
        const autoTitleGenerated = autoGeneratedTitle ? true : node.autoTitleGenerated === true;
        const titleAutoGeneratedAt = autoGeneratedTitle ? Date.now() : node.titleAutoGeneratedAt;
